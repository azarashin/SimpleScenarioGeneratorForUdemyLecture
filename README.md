...
大規模なアウトラインを一度の応答で全面改稿すると章の欠落や番号崩れが起きるため、アウトライン
評価は「全体の物語設計台帳」1回と「章単位」のレビューに分割して実行します。章番号、節番号、
サブセクション番号、イベントIDは元のアウトラインを正としてコード側でも維持します。
章の検証に失敗した場合は、具体的な失敗理由を渡してその章だけを再試行します。成功済みの台帳と
章は`artifacts/outline-review-units/`から再利用されるため、後半章の失敗で前半章を再生成しません。
...
空白を除いて1,200文字を目標、850〜1,600文字を合格範囲として生成します。
