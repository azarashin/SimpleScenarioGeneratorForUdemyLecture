    ...
    def _chapter_prompt(
        *,
        ...
        return (
            ...
            "the opening for convenience. Give every section a concrete scene_location, "
            "scene_activity describing what is currently happening, and scene_phase describing "
            "the scene's narrative stage rather than merely its topic. Give every section "
            "a participant_presence entry for every participating character. Use "
            "location_status_before=not_introduced only at a character's first appearance; "
            "otherwise distinguish a known prior location from an unknown current location. "
            "Use unknown only when an earlier event genuinely makes the character's whereabouts "
            "untracked; do not use it merely to avoid planning a transition. "
            "Use presence_mode=remote for calls, transmissions, memories, or other participation "
            "that does not put the character physically at scene_location. The entry_explanation "
            "must state how an in-person character arrived or remained there, or how a remote "
            "character connects to the scene. For every participant, define a distinct "
            "scene_role, a concrete current_activity compatible with scene_activity, and a "
            "participation_status of active, observing, waiting, or incapacitated. Do not make "
            "every attendee active by default: only characters who directly advance the current "
            "event should be active, while others must have a credible non-speaking or supporting "
            "activity. Before placing a character in "
            ...
            "appearing, but that alone does not make the character a scene participant. When "
            "a known location differs from scene_location, include a plausible movement, "
            "summons, or meeting transition before the character participates in person."
        )
